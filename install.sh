...
__install_keycloak_docker() {
  ...
  # docker compose re-interpolates ${VAR}/$VAR inside the YAML file it reads,
  # so a literal '$' from __random_password's charset followed by identifier
  # characters is silently stripped as an unset-variable reference — corrupting
  # the password with no error at `compose up` time. Escape '$' as '$$' (the
  # compose-file literal-dollar syntax) only in the values written below.
  local kc_admin_password_escaped="${INSTALL_KEYCLOAK_ADMIN_PASSWORD//\$/\$\$}"
  local kc_db_password_escaped="${INSTALL_KEYCLOAK_DB_PASSWORD//\$/\$\$}"
...
services:
  postgres:
    ...
    environment:
      ...
      POSTGRES_PASSWORD: "${kc_db_password_escaped}"
    volumes:
  ...
  keycloak:
    ...
    environment:
      ...
      KC_DB_PASSWORD: "${kc_db_password_escaped}"
      ...
      KC_BOOTSTRAP_ADMIN_PASSWORD: "${kc_admin_password_escaped}"
      KC_LOG_LEVEL: INFO
